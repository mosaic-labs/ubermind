...
exports.getFindQuery = (req) => {
  ...
  const entries = Object.entries(reqQuery)

  let queryProperty = Object.keys(reqQuery)[0]
  if (queryProperty === undefined) return reqQuery

  const regex = /-/g
  let query, queryStatement = reqQuery[queryProperty].replace(regex, " ")

  if (queryStatement.includes('||')) {
    queryStatement = queryStatement.split('||')
    query = { $in: queryStatement }
  }
  if (queryProperty.includes('!')) {
    delete reqQuery[queryProperty]
    queryProperty = queryProperty.replace("!", "")
